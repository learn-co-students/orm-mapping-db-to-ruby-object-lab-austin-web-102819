class Student
  ...
  #method: take row from database and turn it back into student object
  #-accepts a row from database as argument
  #-then creates a new student object based on info in the row
  #-database does not store ruby objects(need to manually convert)
...
#takes in a number of students as argument from grade ten to select
  def self.first_X_students_in_grade_10(num_of_students)
    sql = <<-SQL
    select * from students where grade = 10 limit ?
    SQL
    selected_students = DB[:conn].execute(sql, num_of_students)
    selected_students.map{|student| self.new_from_db(student)}
  end
